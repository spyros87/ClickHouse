...
- `kafka_skip_broken_messages` – Kafka message parser tolerance to schema-incompatible messages. If `kafka_skip_broken_messages = N` then the engine skips *N* Kafka messages that can't be parsed (a message equals a row of data).
